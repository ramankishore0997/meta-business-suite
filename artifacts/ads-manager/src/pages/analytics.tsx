...
import { TrendingUp, Users, Target, MousePointerClick, Activity, CalendarDays } from "lucide-react";
...
export default function AnalyticsPage() {
  ...
  const { data: summary, isLoading: isLoadingSummary } = useGetInsightsSummary(
    { dateRange },
    { query: { queryKey: getGetInsightsSummaryQueryKey({ dateRange }) } }
  );
                ...
                {isLoadingInsights ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} layout="vertical" margin={{ left: 0, right: 0, top: 0, bottom: 0 }}>
                      ...
                      <YAxis dataKey="campaignName" type="category" width={130} interval={0} tickFormatter={(value: string) => value.length > 13 ? `${value.slice(0, 12)}...` : value} tick={{fontSize: 11, fill: 'hsl(var(--foreground))'}} axisLine={false} tickLine={false} />
                      <Tooltip 
